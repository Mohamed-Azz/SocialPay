    ...
    <div class="flex h-screen overflow-hidden">
        ...
        <aside class="w-72 bg-white dark:bg-stone-900 border-l border-slate-200 dark:border-stone-800 flex flex-col h-full shadow-sm">
            ...
            <nav class="flex-1 overflow-y-auto p-4 space-y-2">
                ...
                <a class="flex items-center gap-3 px-4 py-3 rounded-xl bg-primary text-white font-medium transition-colors text-slate-700 dark:text-slate-300" href="<?= URLROOT; ?>/admin/mandates">
        ...
        <main class="flex-1 flex flex-col min-w-0 overflow-hidden">
            <header class="h-16 bg-white dark:bg-stone-900 border-b border-slate-200 dark:border-stone-800 flex items-center justify-between px-8 z-10 shadow-sm">
                <h2 class="text-xl font-bold">إدارة العهد (Mandates)</h2>
            </header>
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-right">
                            <thead class="bg-slate-50 dark:bg-stone-800/50 text-slate-500 text-sm uppercase">
                                <tr>
                                    <th class="px-6 py-4 font-semibold">اسم العهدة</th>
                                    <th class="px-6 py-4 font-semibold">تاريخ البداية</th>
                                    <th class="px-6 py-4 font-semibold">تاريخ النهاية</th>
                                    <th class="px-6 py-4 font-semibold">الميزانية</th>
                            ...
                            <tbody class="divide-y divide-slate-100 dark:divide-stone-800">
                                ...
                                <tr class="hover:bg-slate-50/50 dark:hover:bg-stone-800/30">
                                    <td class="px-6 py-4 font-bold text-sm"><?= $m['name']; ?></td>
                                    <td class="px-6 py-4 text-sm"><?= $m['start_date']; ?></td>
                                    <td class="px-6 py-4 text-sm"><?= $m['end_date']; ?></td>
                                    <td class="px-6 py-4 font-bold text-sm"><?= number_format($m['budget'], 2); ?> دج</td>
                                    <td class="px-6 py-4">
                                        <span class="px-2 py-0.5 rounded-full text-[10px] font-bold
                                            <?= $m['is_active'] ? 'bg-green-100 text-green-800' : 'bg-slate-100 text-slate-800'; ?>">
