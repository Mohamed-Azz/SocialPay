    ...
    <div class="flex h-screen overflow-hidden">
        ...
        <aside class="w-72 bg-white dark:bg-stone-900 border-l border-slate-200 dark:border-stone-800 flex flex-col h-full shadow-sm">
            ...
            <nav class="flex-1 overflow-y-auto p-4 space-y-2">
                <a class="flex items-center gap-3 px-4 py-3 rounded-xl bg-primary text-white font-medium transition-colors text-slate-700 dark:text-slate-300" href="<?= URLROOT; ?>/admin/dashboard">
        ...
        <main class="flex-1 flex flex-col min-w-0 overflow-hidden">
            ...
            <header class="h-16 bg-white dark:bg-stone-900 border-b border-slate-200 dark:border-stone-800 flex items-center justify-between px-8 z-10 shadow-sm">
                <div class="flex items-center gap-4 flex-1">
                    <div class="relative w-full max-w-md">
                        ...
                        <input class="w-full pr-10 pl-4 py-2 rounded-xl bg-slate-100 dark:bg-stone-800 border-none focus:ring-2 focus:ring-primary/50 text-sm" placeholder="بحث..." type="text"/>
                    </div>
                </div>
            ...
            <div class="flex-1 overflow-y-auto p-8 space-y-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    <div class="bg-white dark:bg-stone-900 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-stone-800">
                        <p class="text-slate-500 dark:text-slate-400 text-sm font-medium">إجمالي الطلبات</p>
                        <h3 class="text-3xl font-black mt-1"><?= count($allRequests); ?></h3>
                    </div>
                    <div class="bg-white dark:bg-stone-900 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-stone-800">
                        <p class="text-slate-500 dark:text-slate-400 text-sm font-medium">طلبات مدروسة</p>
                        <h3 class="text-3xl font-black mt-1"><?= count($studiedRequests); ?></h3>
                    </div>
                    <div class="bg-white dark:bg-stone-900 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-stone-800">
                        <p class="text-slate-500 dark:text-slate-400 text-sm font-medium">نسبة التنفيذ</p>
                        <h3 class="text-3xl font-black mt-1">
                            <?= count($allRequests) > 0 ? round((count($studiedRequests) / count($allRequests)) * 100, 1) : 0; ?>%
                        </h3>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 xl:grid-cols-2 gap-8">
                    ...
                    <div class="bg-white dark:bg-stone-900 rounded-2xl shadow-sm border border-slate-100 dark:border-stone-800 overflow-hidden">
                        <div class="p-6 border-b border-slate-100 dark:border-stone-800 flex justify-between items-center">
                            <h3 class="text-lg font-bold">آخر الطلبات (demandes)</h3>
                        </div>
                        <div class="overflow-x-auto">
                            <table class="w-full text-right">
                                ...
                                <tbody class="divide-y divide-slate-100 dark:divide-stone-800">
                                    <?php foreach(array_slice($allRequests, 0, 5) as $r): ?>
                                    <tr class="hover:bg-slate-50/50 dark:hover:bg-stone-800/30">
                                        <td class="px-6 py-4 text-sm font-medium"><?= $r['nom_ar']; ?></td>
                                        <td class="px-6 py-4 text-sm"><?= $r['grant_name']; ?></td>
                                        <td class="px-6 py-4">
                                            <span class="px-2 py-0.5 rounded-full text-xs font-medium
                                                <?= $r['status'] == 'beneficiary' ? 'bg-green-100 text-green-800' : 'bg-orange-100 text-orange-800'; ?>">
                                                <?= $r['status']; ?>
                                            </span>
                                        </td>
                                    </tr>
                    ...
                    <div class="bg-white dark:bg-stone-900 rounded-2xl shadow-sm border border-slate-100 dark:border-stone-800 overflow-hidden">
                        <div class="p-6 border-b border-slate-100 dark:border-stone-800 flex justify-between items-center">
                            <h3 class="text-lg font-bold">الطلبات المسددة (traitement)</h3>
                        </div>
                        <div class="overflow-x-auto">
                            <table class="w-full text-right">
                                <thead>
                                ...
                                <tbody class="divide-y divide-slate-100 dark:divide-stone-800">
                                    <?php foreach(array_slice($studiedRequests, 0, 5) as $p): ?>
                                    <tr class="hover:bg-slate-50/50 dark:hover:bg-stone-800/30">
                                        <td class="px-6 py-4 text-sm font-medium"><?= $p['nom_ar']; ?></td>
                                        <td class="px-6 py-4 text-sm font-bold"><?= number_format($p['amount'], 2); ?> دج</td>
                                        <td class="px-6 py-4 text-sm text-slate-500"><?= $p['payment_date']; ?></td>
                                    </tr>
                                    <?php endforeach; ?>
                                </tbody>
                            </table>
